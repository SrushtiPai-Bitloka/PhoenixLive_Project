defmodule Project1Web.PageLive.FormComponent do
  use Project1Web, :live_component
  ...
  def update(%{page: page} = assigns, socket) do
    changeset = Home.change_page(page)
  ...
  defp save_page(socket, :edit, page_params) do
    case Home.update_page(socket.assigns.page, page_params) do
      {:ok, page} ->
  ...
  defp save_page(socket, :new, page_params) do
    case Home.create_page(page_params) do
      {:ok, page} ->
        notify_parent({:saved, page})
